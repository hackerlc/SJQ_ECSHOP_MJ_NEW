...
import com.ecjia.view.activity.SearchSellerGoodsActivity;
...
import com.ecmoban.android.sijiqing.databinding.ActivityNewShopDetailBinding;
...
import static com.ecmoban.android.sijiqing.R.id.layout_custommenu;
import static com.ecmoban.android.sijiqing.R.id.stick_toolbar;

/**
 * SJQ_ECSHOP_MJ_NEW
 * 商铺详情
 * Created by YichenZ on 2017/4/11 09:35.
 */

public class ShopDetailActivity extends NewBaseActivity implements XListView.IXListViewListener, HttpResponse {
    ...
    ActivityNewShopDetailBinding mBinding;
    com.ecjia.view.activity.shopdetail.GoodsListAdapter mAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mBinding = DataBindingUtil.setContentView(this,R.layout.activity_new_shop_detail);
        PushAgent.getInstance(this).onAppStart();
        headView1 = View.inflate(this, R.layout.layout_shop_info, null);
        initHeadView();
        mBinding.stickToolbar.setVisibility(View.GONE);
        Intent intent = getIntent();
        sellerid = intent.getStringExtra(IntentKeywords.MERCHANT_ID);
        llManager = new LinearLayoutManager(this);
        glManager = new GridLayoutManager(this,2);
        mBinding.goodsListview.setLayoutManager(glManager);
        dataModel = new ShopDetialModel(this);
        mAdapter = new com.ecjia.view.activity.shopdetail.GoodsListAdapter(this,dataModel.simplegoodsList);
        mAdapter.addHeaderView(headView1);
        mBinding.goodsListview.setAdapter(mAdapter.adapter());
        mBinding.goodsListview.setOnScrollListener(new RecyclerView.OnScrollListener() {
            @Override
            public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
                super.onScrollStateChanged(recyclerView, newState);
                if (newState > 1) {
                    mBinding.stickToolbar.setVisibility(View.VISIBLE);
                } else {
        ...
        searchFilter.setOnClickListener(v -> changeLayout());
        tabfour.setOnClickListener(v -> changeLayout());
        shared = getSharedPreferences(SharedPKeywords.SPUSER, 0);

        editor = shared.edit();
        type = shared.getInt(SharedPKeywords.SPUSER_KLISTTYPE, 1);
        Intenthandler = new Handler() {
            public void handleMessage(Message msg) {

                if (msg.obj == ProtocolConst.SHOPFILTER) {
                    if (msg.what == 1) {
                        t3 = true;
                        if (null == catlist) {
                            catlist = new ArrayList();  // 分类数据
                        }
                        if (dataModel.categoryArrayList.size() > 0) {
                            catlist = dataModel.categoryArrayList;
        ...
        backImageButton.setOnClickListener(v -> {
            if (hasFollwerChange) {
                ...
                intent12.putExtra("collect_num", follower + "");
        ...
        shopping_cart.setOnClickListener(v -> {
            String uid1 = shared.getString(SharedPKeywords.SPUSER_KUID, "");
            if (uid1.equals("")) {
                Intent intent1 = new Intent(this, LoginActivity.class);
                startActivity(intent1);
                overridePendingTransition(R.anim.push_buttom_in, R.anim.push_buttom_out);
                String nol = getResources().getString(R.string.no_login);
                ToastView toast1 = new ToastView(this, nol);
        ...
        dataModel.getFilter(sellerid, Intenthandler);
        selectedTab(IS_NEW);
        initMenuRight();
    }
...
//                        + mBinding.stickToolbar.getMeasuredHeight() - top_view.getMeasuredHeight();
//                int t = getScrollY();
//                if (t == 0) {
...
//        });
        ...
        if (url.equals(ProtocolConst.SHOPDETIAL)) {
            ...
            if (size > 0) {
                ...
                if (size > 1) {
                    ...
                    shopDetailFavoursAdapter = new ShopDetailFavoursAdapter(this, list);
    ...
    private void setCustomMenu(ArrayList<MENU_BUTTON> menu_button) {
        if (menu_button != null && menu_button.size() > 0) {
            mBinding.layoutCustommenu.setVisibility(View.VISIBLE);
            mBinding.layoutCustommenu.removeAllViews();
            ...
            for (int i = 0; i < size; i++) {
                ...
                layout.setOnClickListener(v -> {
                    if (ob.getSub_button().size() == 0) {
                        ECJiaOpenType.getDefault().startAction(this, ob.getUrl());
                    } else {
                        popupWindow_custommenu = new PopMenus(this, ob.getSub_button(), v.getWidth() - 20, 0);
                        popupWindow_custommenu.showAtLocation(v);
                    }
                });
                mBinding.layoutCustommenu.addView(layout);
            }
        } else {
            mBinding.layoutCustommenu.setVisibility(View.GONE);
        }
    }

    public int getScrollY() {

        View c = mBinding.goodsListview.getChildAt(0);
        if (c == null) {
            return 0;
        }
        int firstVisiblePosition = mBinding.goodsListview.getVerticalScrollbarPosition();
        int top = c.getTop();
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.search_search:
                Intent intent = new Intent(this, SearchSellerGoodsActivity.class);
            ...
            case R.id.ll_company_phone:
                ...
                if (!TextUtils.isEmpty(number)) {
                    mDialog = new MyDialog(this, call, number);
                    mDialog.show();
            ...
            case R.id.ll_top_category:
                if (t1 & t2 & t3) {
                    ...
                    ToastView toast = new ToastView(this, "网络问题");
                    toast.setGravity(Gravity.CENTER, 0, 0);
                    toast.show();
                }
                break;
    ...
    private void initmenuView() {
        ...
        menulist.setOnChildClickListener((parent, v, groupPosition, childPosition, id) -> {
            ...
            menuadapter.notifyDataSetChanged();
            dataModel.fetchGoodList(this.filter, sellerid);
            menu.toggle();
            return true;
        });

        fliterfinish.setOnClickListener(new View.OnClickListener() {
    ...
    public void changeLayout() {
        if (type == 1) {
            type = 3;
            mBinding.goodsListview.setLayoutManager(llManager);
    ...
    protected void onPause() {
        super.onPause();
        editor.putInt(SharedPKeywords.SPUSER_KLISTTYPE, type);
        editor.commit();
    }
