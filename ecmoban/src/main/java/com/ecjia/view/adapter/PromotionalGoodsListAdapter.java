 ...
 */
public class PromotionalGoodsListAdapter extends BaseAdapter {
    private final int distance;
    private Context context;
    private ArrayList<SUGGEST> list = new ArrayList<SUGGEST>();

    public PromotionalGoodsListAdapter(Context context, ArrayList<SUGGEST> list) {
    ...
    public View getView(final int position, View view, ViewGroup viewGroup) {
        ...
        if(position*2<list.size()){
            left= (SUGGEST) list.get(position*2);
        }

        if (position*2 + 1 < list.size()) {
            right = (SUGGEST) list.get(position*2 + 1);
        }
        ...
        if(left==null){
            ...
            ImageLoaderForLV.getInstance(context).setImageRes(holder.GoodImgLeft, left.getPhoto().getThumb());
            ...
            if (null == right) {
                ...
                ImageLoaderForLV.getInstance(context).setImageRes(holder.GoodImgRight, right.getPhoto().getThumb());
