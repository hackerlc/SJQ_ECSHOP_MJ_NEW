...
public class TopicInfoAdapter extends BaseAdapter {
    ...
    private ArrayList<SELLERGOODS> list;
    private ImageLoaderForLV imageLoader;

    public TopicInfoAdapter(Context context, ArrayList<SELLERGOODS> topic) {
    ...
    public View getView(final int position, View view, ViewGroup viewGroup) {
        ...
        SELLERGOODS left=null;
        SELLERGOODS right=null;
        if(position*2<list.size()){
            left= (SELLERGOODS) list.get(position*2);
        }

        if (position*2 + 1 < list.size()) {
            right = (SELLERGOODS) list.get(position*2 + 1);
        }
        ...
        if(left==null){
            ...
            final SELLERGOODS finalLeft = left;
            ...
            if (null == right) {
                ...
                final SELLERGOODS finalRight = right;
                ...
                holder.llItemRight.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        Intent intent = new Intent();
                        intent.setClass(context, GoodsDetailActivity.class);
                        intent.putExtra("goods_id", finalRight.getId());
                        context.startActivity(intent);
                        ((Activity) context).overridePendingTransition(R.anim.push_right_in,
                                R.anim.push_right_out);
                    }
